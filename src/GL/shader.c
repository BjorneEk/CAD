...
{
	glUniformMatrix4fv(shader_get_uniform(self, name), 1, GL_FALSE, m);
}
void shader_setV3(shader_t * self, const char *name, v3f_t v)
{
	glUniform3f(shader_get_uniform(self, name), v.x, v.y, v.z);
}
void shader_setV4(shader_t * self, const char *name, v4f_t v)
{
	glUniform4f(shader_get_uniform(self, name), v.x, v.y, v.z, v.w);
}
void shader_setV2(shader_t * self, const char *name, v2f_t v)
{
	glUniform2f(shader_get_uniform(self, name), v.x, v.y);
}